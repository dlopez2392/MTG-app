      ...
      <PageContainer>
        ...
        <div className="flex items-center justify-between mb-4">
            ...
            {(["log", "stats"] as const).map((v) => (
              <button key={v} onClick={() => setView(v)}
                className={cn(
                  ...
                  view === v ? "btn-gradient" : "text-text-muted hover:text-text-secondary"
                )}>
                {v}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => router.push("/games/analytics")}
              className="p-2 rounded-xl bg-bg-card border border-border text-text-muted hover:text-accent hover:border-accent/40 transition-colors"
              title="Analytics"
            >
