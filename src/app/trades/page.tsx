...
function CardSearchModal({ onAdd, onClose }: { onAdd: (card: TradeCard) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      <div className="relative w-full max-w-lg max-h-[80vh] bg-bg-card border border-border rounded-t-2xl sm:rounded-2xl flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="p-4 border-b border-border flex items-center gap-3">
          ...
          />
          <button onClick={onClose} className="text-text-muted hover:text-text-primary text-sm font-medium">Cancel</button>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          ...
          {results.map((card) => {
            ...
            return (
              <div key={card.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors">
                ...
                <div className="flex gap-1.5 flex-shrink-0">
                  {price && (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-emerald-500/15 text-emerald-400 text-xs font-semibold hover:bg-emerald-500/25 transition-colors"
                    >
                      ${price}
                    </button>
                  )}
                  {foilPrice && (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-purple-500/15 text-purple-400 text-xs font-semibold hover:bg-purple-500/25 transition-colors"
                    >
                  ...
                  {!price && !foilPrice && (
                    <button
                      onClick={() => handlePick(card, false)}
                      className="px-2.5 py-1 rounded-lg bg-white/10 text-text-muted text-xs font-semibold hover:bg-white/15 transition-colors"
                    >
                      Add
                    </button>
                  )}
